...
$sub_groupsList= $result->fetchAll();
$result = null;
?>
...
<div class="fancy">
    <form onsubmit="return false" autocomplete="off">
        <div id="fancy-inputs">
            <label class="input">
                <input type="text" id="inputSubgroup">
                <span><span>Введите номер подгруппы</span></span><span class="close"><i class="fas fa-times"></i></span>
...
<div id="results">
    <p id="infoParagr">Текущий список подгрупп</p>
    <?
    foreach ($sub_groupsList as $subgroupsItem):?>
        <div class="infoP" id="<?='infoP'.$subgroupsItem['id']?>">
            <p id="<?='p'.$subgroupsItem['id']?>"><?=$subgroupsItem['name']?></p>
        </div>
        <div class="buttonsP" id="<?='buttonsP'.$subgroupsItem['id']?>">
            <button class="update" onclick="is_update(<?=$subgroupsItem['id']?>)" id="<?='update'.$subgroupsItem['id']?>" type="button"><i class="fas fa-pencil-alt"></i></button>
            <button class="del" onclick="del(<?=$subgroupsItem['id']?>);" id="<?=$subgroupsItem['id']?>" type="button"><i class="far fa-trash-alt"></i></button>
        </div>
    <? endforeach;?>
</div>
...
<script>
    ...
    $("#inputSubgroup").keypress(function (e) {
        if (e.which == 13) {
            captcha_generate();
        }
    });
    ...
    function add() {
        var data = {'type': 'insert', 'data': $("#inputSubgroup").val(), 'g-recaptcha-response': $("#g-recaptcha-response").val()};
        $.ajax({
            url: 'handler.php',
            type: "POST",
            dataType: "html",
