...
<div id="results">
    <p id="infoParagr">Текущий список групп</p>
    <?
    foreach ($groupsList as $groupItem):?>
        <div class="infoP" id="<?='infoP'.$groupItem['id']?>">
            <p id="<?='p'.$groupItem['id']?>"><?=$groupItem['name']?></p>
        </div>
        <div class="buttonsP" id="<?='buttonsP'.$groupItem['id']?>">
            <button class="update" onclick="is_update(<?=$groupItem['id']?>)" id="<?='update'.$groupItem['id']?>" type="button"><i class="fas fa-pencil-alt"></i></button>
            <button class="del" onclick="del(<?=$groupItem['id']?>);" id="<?=$groupItem['id']?>" type="button"><i class="far fa-trash-alt"></i></button>
        </div>
    <? endforeach;?>
</div>
...
<script>
    ...
    function add() {
        var data = {'type': 'insert', 'data': $("#inputGroup").val(), 'g-recaptcha-response': $("#g-recaptcha-response").val()};
