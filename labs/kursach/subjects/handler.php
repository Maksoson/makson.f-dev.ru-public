...
if ($_POST['type'] == 'insert') {
    ...
    if ($Return->success == true && $Return->score > 0.5) {
        if (isset($_POST['data'])) {
            $name = $_POST['data'];
            $db = DB::getDb();
            $result = $db->prepare("INSERT INTO subjects (name) VALUES (:subjectName)");
            $result->bindParam(':subjectName', $name);
            $result->execute();
            $id = $db->lastInsertId();
            if ($id) {
                $outputArray = array(
    ...
    if (isset($_POST['data'])) {
        $id = $_POST['data'];
        $db = DB::getDb();
        $result = $db->prepare("DELETE FROM subjects WHERE id = :id");
    ...
    if (isset($_POST['data'])) {
        ...
        $new_name = $_POST['data'];
        $db = DB::getDb();
        $result = $db->prepare("UPDATE subjects SET name = :new_name where id = :id");
