    ...
    <form onsubmit="return false" autocomplete="off">
        <div id="fancy-inputs">
            <label class="input">
                ...
                <span><span>Введите предмет</span></span><span class="close"><i class="fas fa-times"></i></span>
...
<div id="results" style="width: 80%;">
    <p id="infoParagr">Текущий список предметов</p>
    <?
    foreach ($subjectsList as $subjectItem):?>
        <div class="infoP" id="<?='infoP'.$subjectItem['id']?>">
            <p id="<?='p'.$subjectItem['id']?>"><?=$subjectItem['name']?></p>
        </div>
        <div class="buttonsP" id="<?='buttonsP'.$subjectItem['id']?>">
            <button class="update" onclick="is_update(<?=$subjectItem['id']?>)" id="<?='update'.$subjectItem['id']?>" type="button"><i class="fas fa-pencil-alt"></i></button>
            <button class="del" onclick="del(<?=$subjectItem['id']?>);" id="<?=$subjectItem['id']?>" type="button"><i class="far fa-trash-alt"></i></button>
        </div>
    <? endforeach;?>
</div>
...
<script>
    ...
    $("#inputSubject").keypress(function (e) {
        if (e.which == 13) {
            captcha_generate();
        }
    });
    ...
    function add() {
        var data = {'type': 'insert', 'data': $("#inputSubject").val(), 'g-recaptcha-response': $("#g-recaptcha-response").val()};
