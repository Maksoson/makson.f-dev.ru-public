...
if ($_POST['type'] == 'insert') {
    ...
    if ($Return->success == true && $Return->score > 0.5) {
        if (isset($_POST["data"])) {
            $name = $_POST["data"];
            $db = DB::getDb();
            $result = $db->prepare("INSERT INTO cabinets (name) VALUES (:cabinetName)");
            $result->bindParam(':cabinetName', $name);
            $result->execute();
            $id = $db->lastInsertId();
            if ($id) {
                $outputArray = array(
    ...
    if (isset($_POST['data'])) {
        $id = $_POST['data'];
        $db = DB::getDb();
        $result = $db->prepare("DELETE FROM cabinets WHERE id = :id");
    ...
    if (isset($_POST['data'])) {
        ...
        $new_name = $_POST['data'];
        $db = DB::getDb();
        $result = $db->prepare("UPDATE cabinets SET name = :new_name where id = :id");
